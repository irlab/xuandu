...
public class weiboContent extends Activity implements IWeiboActivity, Runnable {
	...
	public void refreshview() {
		try {
			...
			URL url = WeiboUtil
					.getString(status.getUser().getProfileImageURL());
			// 异步加载头像图片
			anseylodar.showimgAnsy(tweet_profile_preview, url.toString());
			if (status.getOriginalPic() != null) {
				// 一步加载内容图片
				anseylodar.showimgAnsy(tweet_upload_pic,
						status.getThumbnailPic());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void refeshtweestatus() {
		...
		try {
			...
			SpannableString ss = new SpannableString(text
					+ tweetstatus.getText());
			ss.setSpan(new ForegroundColorSpan(Color.BLUE), 0, text.length(),
					Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
			tweet_oriTxt.setText(ss);
			// 异步加载图片
			anseylodar.showimgAnsy(tweet_upload_pic2,
					tweetstatus.getThumbnailPic());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	protected void onStart() {
		...
		try {
			status = new Status(new JSONObject((String) data.getExtras().get(
					"status")));
	...
	private void initView() {
		...
		View head = LayoutInflater.from(this).inflate(R.layout.detailweibo2,
				null);

		tweet_profile_preview = (ImageView) head
				.findViewById(R.id.tweet_profile_preview);
		tweet_profile_name = (TextView) head
				.findViewById(R.id.tweet_profile_name);
		...
		tweet_upload_pic2 = (ImageView) head
				.findViewById(R.id.tweet_upload_pic2);
		...
		// 写微博按钮
		ImageView backWeibo = (ImageView) title
		...
		adapter = new commentAdapter(this,
				(List<Comment>) (new ArrayList<Comment>()));
