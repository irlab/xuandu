...
public class weiboContent2 extends Activity implements IWeiboActivity, Runnable {
...
//	View bottom; 
	...
	public void refreshview(){
		try {
			...
			String _retweet_friends = "";
			ArrayList<String> _t = (ArrayList<String>) status.getfriends();
			for(int i = 0; i < _t.size() && i < 2; i++) {
				_retweet_friends = _retweet_friends + "@" +  _t.get(i);
			}
			if(_t.size() > 2)  _retweet_friends += "等";
			
			retweet_friends.setTextColor(this.getResources().getColor(R.color.dark_blue));
			retweet_friends.setText(_retweet_friends);
				...
				tweet_upload_pic.setOnClickListener( new OnClickListener( ) {
					...
					public void onClick(View v) {
						...
						if (pic_path != null) {  
							...
							Intent intent = new Intent( weiboContent2.this, picActivity.class);
						
							intent.putExtra("pic_path", pic_path);
					//	startActivity(intent);
							weiboContent2.this.startActivity(intent);	
						}
					}
				});
	...
	private ColorStateList getResources(int blue) {
...
//			REDIRECT=(BUTTON) THIS.FINDVIEWBYID(R.ID.DETAIL_REDIRECT);
		...
		//weibolist.removeFooterView(bottom);
...
//		});
	...
	public void init() {
		...
		refresh(START_WEIBO, status.getComments());
...
//		titleprogressBar.setVisibility(View.VISIBLE);
	...
	public void refresh(Object... param) {
		...
		case START_WEIBO:
			...
			List<Comment> _nowStatus = (List<Comment>) (new ArrayList<Comment>()); 
			titleprogressBar.setVisibility(View.GONE);
			loginprogress.setVisibility(View.GONE);
			comment_show_num += _nowStatus.size();
			adapter = new commentAdapter(this, _nowStatus);
			weibolist.setAdapter(adapter);
			adapter.notifyDataSetChanged();
			
			refresh(REFRESH_WEIBO, nowStatus);

		//	bottom.setVisibility(View.GONE);
			break;
//			if(nowStatus.size() == 0 || nowStatus.get(0).getStatus().getCommentsCount() == comment_show_num) {
//				flag = 1;
//				//	weibolist.removeFooterView(bottom);
//			}
		}
